...
## 当前状态
- 状态：进行中
- 当前阶段：快速项清理（T21 完成，工作区干净）
- 最近更新：2026-08-02
- 当前分支：master（v2.0 已合并，feature 分支已删）
- 当前主任务 ID：T21（已 DONE），下一步 T20/T22 快速项
...
## 下一步（按优先级）
1. T20（本地化 Google 字体，完全离线可用）——依赖网络问题，适合快速清尾
2. T22（添加取经进度指示，如"第23/31难·狮驼岭"）——注意：T23 时间轴已实现进度条，需评估 T22 是否已被覆盖或仍有增量
3. T25（真实中国—西域地图底图 + D3 投影路线）——v2.0 唯一剩余扩展，复杂度高、涉及底图数据源/许可，建议单独讨论方案
4. 检查 v2.0 三功能实际运行效果（浏览器确认）
