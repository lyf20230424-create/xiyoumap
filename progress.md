...
- 当前主任务 ID：T23（已 DONE），下一步 T24 或快速项
...
## 下一步（按优先级）
1. 检查 T23 联动功能实际运行效果（需浏览器确认：无头浏览器不可用，静态验证已全通过）
2. T24（妖怪族谱关系图，v2.0 扩展）或 T20/T21/T22（快速项）
3. 查看 README.md（未跟踪文件）是否需要完善更新
4. 清理未跟踪的测试/调试 HTML 文件（需确认保留或删除）
